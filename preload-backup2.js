...
function renderSurveyResult(data) {
    ...
    data.forEach(row => {
        ...
        var surveyHtml = `
            <p style="font-family:Inter,Arial,Sans-serif;font-size:18px;font-weight:600;letter-spacing:-.6px;color:#000;margin-bottom:20px;">${index}&nbsp ${que}</p>
        `;
        ...
        for (var key in ans) { // Correct the for loop syntax
            var answerHtml = `
                    ...
                    <div style="word-break:break-word;padding:0;vertical-align:top;clear:both;display:block;margin-bottom:4px;">${ans[key][0]}</div>
